...
结合傅里叶积分公式以及傅里叶变换的复数形式可以看到，所谓的频域函数$F(\omega)$其实是无穷级数中的系数$c_n$，唯一的一串$c_n$序列决定了一个周期函数$f(t)$，而$c_n$中含有一个完整的正弦函数（谐波）。从(1)式中可以知道，某一个周期函数的第n次谐波为
$$
...
$$
从(8)式中的傅里叶变换的复数形式中我们又可以知道，第n次谐波为
$$
